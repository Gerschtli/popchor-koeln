    ...
    function setCurrentIndex() {
        for (let i = 0; i < sliderElement.children.length; i++) {
            ...
            console.log('setCurrentIndex', {
                i,
                leftEle: ele.getBoundingClientRect().left,
                leftSlider: sliderElement.getBoundingClientRect().left,
            });
...
<div>
    <div class="relative">
        ...
        >
            {#each items as item}
                <div class="grid aspect-video w-full flex-shrink-0 snap-center place-content-center">
                    <slot {item} />
                </div>
            {/each}
        </div>
